...
const Header = () => {
  ...
  const hospitalitySubLinks = [
    ...
    { label: lang === "ar" ? "كافيه الطابق الخامس" : "5th cafe", href: "/fifth-floor-cafe", icon: Coffee, desc: lang === "ar" ? "مساحة مريحة للقهوة والمرطبات الخفيفة" : "A cozy space for light bites and refreshments" },
        ...
        <AnimatePresence>
          {menuOpen && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.2 }}
              className="lg:hidden bg-popover border-t border-border max-h-[calc(100dvh-6.5rem)] overflow-y-auto overscroll-y-contain [-webkit-overflow-scrolling:touch]"
            >
